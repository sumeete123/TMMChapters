...
// Applicant- and volunteer-supplied addresses end up inside mailto:/tel: URLs.
// Percent-encoding the address stops a value like
// "lead@example.com?bcc=someone-else@example.com" from injecting extra mail
// headers when an admin clicks through. "@" is restored because it is legal
// unencoded in a mailto target and keeps the link readable.
function mailtoHref(address: string, params: Record<string, string> = {}) {
  const target = encodeURIComponent(address.trim()).replaceAll("%40", "@");
  const query = Object.entries(params)
    .map(([key, value]) => `${key}=${encodeURIComponent(value)}`)
    .join("&");
  return `mailto:${target}${query ? `?${query}` : ""}`;
}
...
function Status({ value, label }: { value: string; label?: string }) {
  return <span className={`status status-${value.replaceAll("_", "-")}`}>{(label ?? value).replaceAll("_", " ")}</span>;
}

// An application moves to "reviewing" when an admin contacts the applicant with
// questions, so the queue reads as the state it is actually in: waiting on a
// reply rather than untouched. The stored value stays "reviewing" so the open
// -application filters and the one-open-chapter-per-place indexes keep counting it.
function applicationStatusLabel(status: string) {
  return status === "reviewing" ? "reached out" : status;
}
  ...
  return <section className="form-page">
    ...
    <form className="surface-form" onSubmit={onSubmit}>
      ...
      <div className="form-grid">
        ...
        <Field label="City"><input name="city" minLength={2} maxLength={120} placeholder={chapterScope === "school" ? "Raleigh" : "Austin"} required /></Field>
        {chapterScope === "school"
          ...
          : <Field label="State, province, or country" hint="Used to distinguish cities with the same name."><input name="region" minLength={2} maxLength={120} placeholder="Texas" required /></Field>}
...
function AdminView({ data, ready, tab, setTab, onLogin, onAction, onLogout, issuedCode, setIssuedCode, onCaptcha, busy }: { data: AdminData; ready: boolean; tab: AdminTab; setTab: (tab: AdminTab) => void; onLogin: (event: FormEvent<HTMLFormElement>) => void; onAction: (action: string, payload: Record<string, unknown>, codeName?: string) => Promise<void>; onLogout: () => void; issuedCode: IssuedCode | null; setIssuedCode: (value: IssuedCode | null) => void; onCaptcha: (token: string) => void; busy: boolean }) {
  ...
  const missingReports = reportingChapters.filter((chapter) => !currentWeekReportChapters.has(chapter.id));
  ...
  const nationalReport = nationalChapter
    ? data.reports.find((report) => report.chapter_id === nationalChapter.id && report.week_start === thisMonday())
    : undefined;
  const nationalReview = nationalReport ? reviewByReport.get(nationalReport.id) : undefined;
  ...
  const openNationalReport = () => {
    setTab("reviews");
    window.setTimeout(() => document.getElementById("national-weekly-report")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  };
  const exportReviews = () => {
  ...
  return <section className="admin-layout">
    ...
    <div className="admin-content">
      ...
      {tab === "applications" && <section className="admin-section"><div className="section-title"><div><h2>Chapter applications</h2><p>Open applications stay visible. Approved and declined records are hidden by default.</p></div>{closedApplications.length > 0 && <button className="visibility-toggle" onClick={() => setShowClosedApplications((value) => !value)}>{showClosedApplications ? "Hide closed" : `Show ${closedApplications.length} closed`}</button>}</div><div className="application-list">{visibleApplications.length ? visibleApplications.map((application) => <article className="application-card" key={application.id}><div className="application-top"><div><strong>{application.organization_name}</strong><span>{application.location}</span></div><Status value={application.status} label={applicationStatusLabel(application.status)} /></div><div className="application-meta"><span>{application.chapter_scope === "school" ? "North Carolina school chapter" : "Regional city chapter"}</span><span>Primary lead: {application.contact_name}</span>{application.contact_grade && <span>Grade: {application.contact_grade}</span>}<a href={mailtoHref(application.contact_email)}>{application.contact_email}</a>{application.contact_phone && <a href={telHref(application.contact_phone)}>{application.contact_phone}</a>}<span>{new Date(application.created_at).toLocaleDateString()}</span></div>{application.additional_contacts?.length ? <div className="applicant-team"><span className="section-kicker">Additional team members</span>{application.additional_contacts.map((contact, index) => <div key={`${contact.email}-${index}`}><strong>{contact.full_name}</strong><span>{contact.role || "Volunteer"}</span>{contact.email && <a href={mailtoHref(contact.email)}>{contact.email}</a>}{contact.phone && <span>{contact.phone}</span>}</div>)}</div> : null}{application.why && <p>{application.why}</p>}<div className="row-actions">{application.status !== "approved" && <Button disabled={busy} onClick={() => onAction("admin-approve-application", { application_id: application.id }, application.organization_name)}>Approve & prepare email</Button>}{application.status === "new" && <Button kind="secondary" disabled={busy} onClick={() => onAction("admin-update-application", { application_id: application.id, status: "reviewing" })}>Reached out with questions</Button>}{application.status === "reviewing" && <Button kind="quiet" disabled={busy} onClick={() => onAction("admin-update-application", { application_id: application.id, status: "new" })}>Undo reached out</Button>}{application.status !== "declined" && application.status !== "approved" && <Button kind="danger" disabled={busy} onClick={() => onAction("admin-update-application", { application_id: application.id, status: "declined" })}>Reject</Button>}{application.status === "declined" && <Button kind="danger" disabled={busy} onClick={() => window.confirm(`Permanently delete the declined application from ${application.organization_name}?`) && onAction("admin-delete-application", { application_id: application.id })}>Delete</Button>}</div></article>) : <Empty text={closedApplications.length ? "No applications need a decision. Use Show closed to view older records." : "No applications yet."} />}</div></section>}

      {tab === "reviews" && <>
        <div className="metric-strip admin-metrics">
          <div><span>Submitted this week</span><strong>{currentWeekReportChapters.size}</strong><small>of {reportingChapters.length} active chapters</small></div>
          ...
          <div><span>Missing this week</span><strong>{missingReports.length}</strong><small>Needs a report or reminder</small></div>
        </div>
        {nationalChapter && <section className="admin-section national-admin-section weekly-section" id="national-weekly-report">
          <div className="section-title"><div><span className="section-kicker">Official chapter · due every Sunday</span><h2>Submit the National Chapter weekly report</h2><p>The National Chapter is tracked in the same weekly workflow as every active chapter. Administrators submit its report here because it does not use a chapter access code.</p></div><div className="review-meta">{nationalReport && <span className={`review-pill ${nationalReview?.status ?? "pending"}`}>{reviewLabel(nationalReview?.status)}</span>}<span className="sunday-chip"><span>Due</span><strong>{longDate(weekDueDate())}</strong><small>{dueTiming(weekDueDate())}</small></span></div></div>
          <form className="surface-form" onSubmit={submitNationalReport}>
            <div className="form-grid four"><Field label="Week starting"><input type="date" name="week_start" defaultValue={thisMonday()} required /></Field><Field label="Sessions held"><input type="number" name="sessions_held" min="0" max="1000" defaultValue={nationalReport?.sessions_held ?? 0} required /></Field><Field label="People impacted"><input type="number" name="students_served" min="0" max="100000" defaultValue={nationalReport?.students_served ?? 0} required /></Field><Field label="Instructional hours"><input type="number" name="instructional_hours" min="0" max="1000" step="0.25" defaultValue={nationalReport?.instructional_hours ?? 0} required /></Field></div>
            <label className="check-row"><input type="checkbox" name="completed_weekly_tasks" defaultChecked={nationalReport?.completed_weekly_tasks ?? false} /><span>National Chapter weekly priorities are complete.</span></label>
            <div className="form-grid"><Field label="What did the National Chapter accomplish?"><textarea name="highlights" rows={4} defaultValue={nationalReport?.highlights ?? ""} placeholder="Sessions, outreach, curriculum work, wins…" required /></Field><Field label="What challenges came up?"><textarea name="blockers" rows={4} defaultValue={nationalReport?.blockers ?? ""} placeholder="Attendance, scheduling, materials…" /></Field><Field label="What is planned for next week?"><textarea name="next_week_plan" rows={4} defaultValue={nationalReport?.next_week_plan ?? ""} placeholder="Goals, sessions, outreach, deadlines…" required /></Field><Field label="What support is needed?"><textarea name="support_needed" rows={4} defaultValue={nationalReport?.support_needed ?? ""} placeholder="Optional — resources, advice, introductions…" /></Field></div>
            <div className="form-footer"><p>{nationalReport ? `Last saved ${new Date(nationalReport.submitted_at).toLocaleString()}. Saving again returns it to the review queue.` : "This report will enter the same review queue as every other chapter report."}</p><Button type="submit" disabled={busy}>{busy ? "Saving…" : nationalReport ? "Update National report" : "Submit National report"}</Button></div>
          </form>
        </section>}
        <section className="admin-section">
          ...
          {missingReports.length ? <div className="missing-list">{missingReports.map((chapter) => <div key={chapter.id}><span><strong>{chapter.name}</strong><small>{chapter.is_official ? "Admin-managed · submit above" : `${chapter.contact_name} · ${chapter.contact_email}`}</small></span>{chapter.is_official ? <Button kind="secondary" onClick={() => document.getElementById("national-weekly-report")?.scrollIntoView({ behavior: "smooth", block: "start" })}>Complete report</Button> : <a className="button secondary" href={mailtoHref(chapter.contact_email, { subject: "TMM weekly report reminder", body: `Hi ${chapter.contact_name},\n\nYour TMM chapter weekly report for the week of ${new Date(`${thisMonday()}T12:00:00`).toLocaleDateString()} is still due. Please sign in to the chapter portal and submit it by ${weekDueDate().toLocaleDateString()}.\n\nThank you!` })}>Email reminder</a>}</div>)}</div> : <Empty text="Every active chapter has submitted this week." />}
        </section>
        <section className="admin-section">
        ...
        <section className="admin-section">
          ...
          {data.chapters.length ? <div className="chapter-list">{data.chapters.map((chapter) => {
            ...
            return <article className={`chapter-row ${chapter.is_official ? "official" : ""}`} key={chapter.id}>
              ...
              <div><span>{chapter.contact_name}</span><a href={mailtoHref(chapter.contact_email)}>{chapter.contact_email}</a>{chapter.contact_phone && <span>{chapter.contact_phone}</span>}</div>
