...
function ApplicationView({ onSubmit, busy, authState, authMessage, onCaptcha }: { onSubmit: (event: FormEvent<HTMLFormElement>) => void; busy: boolean; authState: AuthState; authMessage: string; onCaptcha: (token: string) => void }) {
  ...
  return <section className="form-page">
    ...
    <form className="surface-form" onSubmit={onSubmit}>
      ...
      <fieldset className="photo-consent" aria-describedby="photo-consent-requirement">
        ...
        <div className="photo-consent-grid">
          ...
          <label className={photoConsent === "no" ? "selected refused" : ""}>
            <input type="radio" name="instagram_photo_consent" value="no" checked={photoConsent === "no"} onChange={() => setPhotoConsent("no")} />
        ...
        {consentBlocked && <p className="photo-consent-block" role="alert">Answering “Yes” is required before you can submit this application or start a chapter.</p>}
      </fieldset>
      ...
      {turnstileSiteKey && authState !== "ready" && <TurnstileChallenge onToken={onCaptcha} />}
      {authMessage && <p className={`access-state ${authState}`}>{authMessage}</p>}
      <div className="form-footer"><p>Applications are reviewed manually. Approved chapters receive a private access code. Instagram photo permission must be answered Yes before you can send this application.</p><Button type="submit" disabled={busy || authState !== "ready" || photoConsent !== "yes"}>{busy ? "Sending…" : authState === "loading" ? "Preparing secure form…" : consentBlocked ? "Instagram consent required" : "Send application"}</Button></div>
    </form>
  </section>;
}
        ...
        <section className="admin-section">
          ...
          {data.chapters.length ? <div className="chapter-list">{data.chapters.map((chapter) => {
            ...
            return <article className={`chapter-row ${chapter.is_official ? "official" : ""}`} key={chapter.id}>
              ...
              <div className="chapter-row-end">{chapter.is_official ? <><span className="status status-approved">Official · admin only</span><Button kind="secondary" onClick={openNationalReport}>Weekly report</Button></> : <><Status value={chapter.status} /><span className="code-hint">Code <code>{chapter.access_code ?? (chapter.access_code_hint ? `••••${chapter.access_code_hint}` : "—")}</code></span>{chapter.access_code && <Button kind="quiet" disabled={busy} onClick={() => navigator.clipboard.writeText(chapter.access_code!)}>Copy</Button>}<Button kind="secondary" disabled={busy} onClick={() => onAction("admin-reset-code", { chapter_id: chapter.id }, chapter.name)}>Reset code</Button><Button kind="danger" disabled={busy} onClick={() => window.confirm(`Delete ${chapter.name}? This permanently removes the chapter, its access code, volunteers, reports, tasks, events, and executive records.`) && onAction("admin-delete-chapter", { chapter_id: chapter.id }, chapter.name)}>Delete chapter</Button></>}</div>
            </article>;
